...
class aoa_info(object):
    ...
    def iteration(self, event):
        webcam_01 = cv2.VideoCapture('udp://@0.0.0.0:8554?overrun_nonfatal=1&fifo_size=50000000')
        ret, image = webcam_01.read()
        mtx = np.array([[2.53417999e+03, 0.00000000e+00, 2.91862100e+02],
                        [0.00000000e+00, 8.29640895e+03, 3.58668921e+02],
                        [0.00000000e+00, 0.00000000e+00, 1.00000000e+00]])
        dist=  ([-1.11959409 , 2.9708864 ,  0.02516476 , 0.07569048 ,-4.09409837])
        img = cv2.undistort(image, mtx, dist, None)
        
        self.u, self.v = hsv.process_image(img)

        if len(azimuth)==0 :
            while(time.time()<=start_time+5):
                w1=[-10,0]
                self.waypoint_command(w1,0)
                #print('-----time----------')
